...
data = {
    ...
    'VLF_power': [VLF_power[i] for i in range(len(VLF_power)-23)], #les saco las ultimas xq no se calculan las ultimas temporales
    'LF_power': [lf_power[i] for i in range(len(lf_power)-23)], #probablemente se haya desconectado el cable canal y de error 
    'HF_power': [hf_power[i] for i in range(len(hf_power)-23)], #la cagada es que son ictales
    'sd1': [sd1[i] for i in range(len(sd1)-23)],
    'sd2': [sd2[i] for i in range(len(sd2)-23)],
    'sd_ratio': [sd_ratio[i] for i in range(len(sd_ratio)-23)],
    'elipse_area': [elipse_area[i] for i in range(len(elipse_area)-23)],
    'clasificacion': [clasificacion[i] for i in range(len(clasificacion)-23)]
}
#%% Agrego una hoja al archivo excell
df = pd.DataFrame(data)
with pd.ExcelWriter('parametros_clasificacion.xlsx', mode='a', engine='openpyxl') as writer:
    df.to_excel(writer, sheet_name='P3SZ2', index=False)