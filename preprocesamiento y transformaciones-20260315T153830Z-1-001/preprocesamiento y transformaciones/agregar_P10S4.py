...
muestra_inicial_preictal3 = (segundos_inicio_crisis3 - 15*60) * frecuencia_muestreo
muestra_final_preictal3 = segundos_fin_crisis3 * frecuencia_muestreo
...
for i in range(0, len(rpeaks), ventana_len - ventana_solap):
    ...
    if (len(ventana_rpeaks) == 180):
        ...
        if (
    i + 180 < len(rpeaks) and (
        (rpeaks[i] > muestra_inicial_preictal1 and rpeaks[i + 180] < muestra_final_crisis1 + 15 * 60 * frecuencia_muestreo) or
        (rpeaks[i] > muestra_inicial_preictal2 and rpeaks[i + 180] < muestra_final_crisis2 + 15 * 60 * frecuencia_muestreo) or
        (rpeaks[i] > muestra_inicial_preictal3 and rpeaks[i + 180] < muestra_final_crisis3 + 15 * 60 * frecuencia_muestreo)
    )
):
...
with pd.ExcelWriter('parametros_clasificacion.xlsx', mode='a', engine='openpyxl') as writer:
    df.to_excel(writer, sheet_name='P10SZ4', index=False)